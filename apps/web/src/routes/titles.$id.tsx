  ...
  return (
    <div className="space-y-8">
      <header className="flex gap-6">
        <div className="w-32 shrink-0 overflow-hidden rounded bg-neutral-800 sm:w-40">
          {poster ? (
        ...
        <div className="min-w-0 space-y-2">
          ...
          {title.excluded ? (
            <p className="text-sm text-neutral-500">Excluded from the wall.</p>
          ) : null}
        </div>
      </header>
...
function TitleError({ error }: ErrorComponentProps) {
  if (error instanceof ApiError && error.status === 401) {
    return <p className="text-neutral-400">Sign in to see this title.</p>;
  }
