...
- ~~The rail carries HOME, LIST and ADD.~~ `YEAR` joined it 2026-09-22 with
  its screen, [web-depth.md](web-depth.md) arc 4 chunk 3.
...
6. ~~**Web depth**~~ — done 2026-09-23, every arc built and deployed.
   ...
   episode ahead is not, label unchanged. Chunk 3, *Hours as a figure*,
   landed 2026-09-22 in two commits (migration 0013, a film's `runtime_min`,
   ...
   collection as tiles where a show's grid sits); deployed and refreshed on
...
8. ~~**Add search**~~ — done 2026-09-24, built and deployed. Scoped
   ...
   search as the owner types. No migrations. Chunks 1 to 6 landed
   2026-09-23 — the wall's kind control and a year field in `/add`'s
   search bar, each kind asking TMDB's own search, TMDB's later pages on
   request, each result's country and original name, *Want* beside *Add* on
...
9. **Visual finish** — scoped 2026-09-24 in [visual-finish.md](visual-finish.md);
   chunks 1 to 7 landed the same day, not deployed. A review of the running app against
   ...
   ticked box in jade. Chunk 2: every value and landmark off `--faint`, so no
   text on a flat ground sits under 4.5:1 but the separators and the cells
   receding on purpose. Chunk 3: a filled missing cell, a `--line` that can be
